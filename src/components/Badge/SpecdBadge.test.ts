...
beforeAll(async () => {
  await import('./SpecdBadge');
});

async function makeElement(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('specd-badge') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('SpecdBadge', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('specd-badge')).toBeDefined();
  });
