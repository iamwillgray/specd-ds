...
beforeAll(async () => {
  await import('./AdmiralCard');
});

async function makeElement(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('admiral-card') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('AdmiralCard', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('admiral-card')).toBeDefined();
  });
  ...
  it('does not render card-inner when inner is false', async () => {
    const el = document.createElement('admiral-card') as HTMLElement & { updateComplete: Promise<boolean>; inner: boolean };
    el.inner = false;
    document.body.appendChild(el);
    await el.updateComplete;
