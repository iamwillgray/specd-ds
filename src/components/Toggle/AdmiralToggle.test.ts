...
beforeAll(async () => {
  await import('./AdmiralToggle');
});

async function makeElement(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('admiral-toggle') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('AdmiralToggle', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('admiral-toggle')).toBeDefined();
  });
