...
beforeAll(async () => {
  await import('./AdmiralChip');
});

async function makeElement(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('admiral-chip') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('AdmiralChip', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('admiral-chip')).toBeDefined();
  });
