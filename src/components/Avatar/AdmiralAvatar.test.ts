...
beforeAll(async () => {
  await import('./AdmiralAvatar');
});

async function makeElement(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('admiral-avatar') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('AdmiralAvatar', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('admiral-avatar')).toBeDefined();
  });
