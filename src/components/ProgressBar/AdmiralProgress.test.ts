...
beforeAll(async () => {
  await import('./AdmiralProgress');
});

async function makeElement(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('admiral-progress') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('AdmiralProgress', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('admiral-progress')).toBeDefined();
  });
