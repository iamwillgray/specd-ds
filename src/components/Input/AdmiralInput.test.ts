...
beforeAll(async () => {
  await import('./AdmiralInput');
});

async function makeElement(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('admiral-input') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('AdmiralInput', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('admiral-input')).toBeDefined();
  });
