...
beforeAll(async () => {
  await import('./AdmiralButton');
});

async function makeButton(attrs: Record<string, string> = {}): Promise<HTMLElement> {
  const el = document.createElement('admiral-button') as HTMLElement & { updateComplete: Promise<boolean> };
...
describe('AdmiralButton', () => {
  it('registers as a custom element', () => {
    expect(customElements.get('admiral-button')).toBeDefined();
  });
